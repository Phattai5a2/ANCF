# -*- coding: utf-8 -*-
"""
Created on Mon Feb 10 23:10:16 2025
...
# Đọc dataset
file_path = 'ratings_edit.csv'  # Đường dẫn tới file của bạn
...
def evaluate_autoencoder(autoencoder, test_user, test_item, test_ratings):
    with torch.no_grad():
        ...
        # Kiểm tra kích thước
        print(f"autoencoder_output shape: {autoencoder_output.shape}")  # Debugging
        print(f"test_item shape: {test_item.shape}")  # Debugging

        # Giới hạn giá trị của test_item trong phạm vi hợp lệ
        max_item_index = autoencoder_output.shape[1] - 1
        test_item = torch.clamp(test_item, 0, max_item_index)  

        # Dùng indexing thay vì gather để tránh lỗi
        pred_ratings_autoencoder = autoencoder_output[torch.arange(autoencoder_output.shape[0]), test_item]

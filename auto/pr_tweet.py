...
user_item_matrix = torch.zeros((len(np.unique(user_ids)), input_dim), dtype=torch.float32).to(device)
...
class HybridModel(nn.Module):
    ...
    def forward(self, user, item, user_item_matrix):
        ...
        combined_output = 0.5 * ncf_output + 0.5 * autoencoder_item_output
        return combined_output

# Huấn luyện và đánh giá mô hình lai (Hybrid)
...
for epoch in range(num_epochs):
    ...
    print(f'Hybrid Model Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}')
...
with torch.no_grad():
    test_user_item_matrix = torch.zeros((len(np.unique(user_ids)), input_dim), dtype=torch.float32).to(device)
...
# Thêm dữ liệu tweet vào DataFrame
tweet_data = df.set_index(['ID', 'Categories'])['Tweet'].unstack().fillna('')
user_item_matrix_df.columns = tweet_data.columns
user_item_matrix_df = user_item_matrix_df.combine_first(tweet_data)

# Thử nghiệm khuyến nghị cho một người dùng cụ thể
sample_user_id = user_item_matrix_df.index[0]
recommended_tweets_indices = recommend_tweets(sample_user_id, hybrid_model, user_item_matrix_df)
recommended_tweets = tweet_data.iloc[sample_user_id, recommended_tweets_indices]

print(f"Recommended tweets for user {sample_user_id}:")
